...
class WinNumberOfUserLNKFiles(MDPPlugin):
    name = 'win_no_user_lnk_files'
    description = 'Number of LNK files within the user folder (all users)'
    expected_results = ['no_lnk_files_in_user_folders', 'no_recent_lnk_max', 'no_recent_lnk_total',
                        'no_start_menu_lnk_max', 'no_start_menu_lnk_total']
    ...
    def process_disk(self, target_disk_image: TargetDiskImage):
        ...
        if home_folder_base_path and windows_path_exists:
            ...
            for each_user in user_folders:
                if home_folder_base_path == 'Users':
                    recent_reg_ex = 'P_[0-9]+/.*{}/AppData/Roaming/Microsoft/Windows/Start Menu/.*\\.lnk$'.format(
                        each_user)
                else:
        ...
        self.set_results(result, {'no_lnk_files_in_user_folders': count,
